...
type Snapshot = {
  status: string;
  refreshCount: number;
  rows: Array<{ title: string; done: number }>;
};
...
function pair(
  value: unknown,
  expected: Array<{ title: string; done: number }>,
) {
...
export const persistedTodos = [
  { title: "active-filter", done: 0 },
  { title: "after-close", done: 0 },
  { title: "after-reload", done: 0 },
  { title: "completed-filter", done: 1 },
];
export function assertTodoPersistence(windows: unknown): void {
  pair(windows, persistedTodos);
}
export function assertTodoAcceptance(
  title: string,
  ...
  deepStrictEqual(rows(stages.filters[1]), [
    { title: "completed-filter", done: 1 },
  ]);
  deepStrictEqual(
    rows(stages.afterReload[0]),
    persistedTodos.filter((row) => row.title !== "after-close"),
  );
  deepStrictEqual(rows(stages.afterReload[1]), [
    { title: "completed-filter", done: 1 },
  ]);
  ...
  equal(
    stages.beforeRollback.refreshCount,
    stages.afterCloseAndRollback.refreshCount,
