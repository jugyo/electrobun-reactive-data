...
import {
  ...
  type Envelope,
  type InvokeParams,
  type SetQueriesParams,
} from "../protocol.js";

export interface SessionEndpoint {
  sendChanged(message: ChangedMessage): void;
}
type Session = {
  token: string;
  revision: number;
...
export class SessionHub {
  ...
  constructor(
    ...
    private readonly mutate: (run: () => unknown) => unknown,
  ) {}

  connect(
    endpoint: SessionEndpoint,
    protocolVersion: number,
  ): Envelope<{ session: string }> {
    if (this.stopping)
      return fail("STOPPING", "Reactive data is stopping") as Envelope<{
        session: string;
      }>;
  ...
  invoke(endpoint: SessionEndpoint, params: InvokeParams): Envelope {
    ...
    if (params.kind !== "query" && params.kind !== "mutation")
      return fail("INVALID_INPUT", "Invalid operation kind");
    try {
      assertWireValue(params);
    } catch (error) {
      return fail(
        "INVALID_INPUT",
        error instanceof Error ? error.message : String(error),
    ...
    if (!own(group, params.name))
      return fail(
        "UNKNOWN_OPERATION",
        `Unknown ${params.kind}: ${params.name}`,
      );
  ...
  ): Envelope<{ revision: number; queries: string[] }> {
    const session = this.current(endpoint, params.session);
    if (!session)
      return fail(
        "STALE_SESSION",
        "The renderer session is stale",
      ) as Envelope<{ revision: number; queries: string[] }>;
    if (
      !Number.isSafeInteger(params.revision) ||
      params.revision < 0 ||
      !Array.isArray(params.queries) ||
      params.queries.length > MAX_QUERIES ||
      params.queries.some(
        (id) =>
          typeof id !== "string" ||
          id.length > 200 ||
          !id.startsWith("query.") ||
          !own(this.api.query, id.slice(6)),
      )
    ) {
      return fail(
        "INVALID_INPUT",
        "Malformed query subscription set",
      ) as Envelope<{ revision: number; queries: string[] }>;
    }
  ...
  detach(endpoint: SessionEndpoint): void {
    const token = this.generations.get(endpoint);
    if (token) this.sessions.delete(token);
    this.generations.delete(endpoint);
  }
  notify(triggerIds: readonly string[]): void {
    const affected = new Set(
      Object.entries(this.api.query)
        .filter(([, query]) =>
          query.dependsOn.some((table) => triggerIds.includes(table)),
        )
        .map(([name]) => `query.${name}`),
    );
    // Snapshot sessions: send callbacks may detach a session during fan-out.
    // oxlint-disable-next-line unicorn/no-useless-spread
    for (const session of [...this.sessions.values()]) {
      const queries = [...session.queries].filter((id) => affected.has(id));
      if (!queries.length) continue;
      try {
        session.endpoint.sendChanged({ session: session.token, queries });
      } catch {
        /* another session still receives its message */
      }
    }
  }
  stop(): void {
  ...
  private current(
    ...
    return this.generations.get(endpoint) === token
