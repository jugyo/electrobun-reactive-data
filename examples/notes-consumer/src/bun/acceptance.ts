...
function pair(
  value: unknown,
  expected: Array<{ title: string; body: string }>,
): void {
  const snapshots = value as Array<{
    status: string;
    refreshCount: number;
  ...
  for (const snapshot of snapshots) {
    ...
    deepStrictEqual(
      snapshot.rows.map(({ title, body }) => ({ title, body })),
      expected,
    );
  }
}
const persisted = [{ title: "native note", body: "edited body" }];
export function assertNotesPersistence(windows: unknown): void {
  pair(windows, persisted);
}
